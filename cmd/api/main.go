...
import (
	...
	authpkg "github.com/gjovanovicst/auth_api/internal/auth"
...
func main() {
	...
	// Wire inactivity resolver into session service now that settingsService is available.
	sessionService.InactivityResolver = &inactivityResolver{db: database.DB, settings: settingsService}
	...
	protected.Use(middleware.AuthMiddleware())
	{
		...
		protected.GET("/2fa/status", middleware.AuthorizePermission(rbacService, "settings", "read"), twofaHandler.Get2FAStatus)
		// Email 2FA routes
	...
	reissueHandler := authpkg.NewReissueHandler()
	...
	appRoutes.Use(middleware.AppRouteGuardMiddleware())
	{
		...
		// Internal: token reissue with org context (called by Centrora after context-switch)
		// SECURITY: never expose this route through a public gateway.
		appRoutes.POST("/auth/reissue", reissueHandler.ReissueToken)

		// Internal: user lookup by email (called by Centrora during the invite flow)
		// SECURITY: never expose this route through a public gateway.
		appRoutes.GET("/users/by-email", userHandler.LookupByEmail)
	}
...
// inactivityResolver implements session.InactivityResolver.
// Checks per-app inactivity_timeout_minutes first; falls back to global setting.
type inactivityResolver struct {
	db       *gorm.DB
	settings interface {
		GetResolvedValue(key string) string
	}
}

func (r *inactivityResolver) GetInactivityTimeoutMinutes(appID string) int {
	var mins int
	r.db.Raw("SELECT inactivity_timeout_minutes FROM applications WHERE id = ? LIMIT 1", appID).Scan(&mins)
	if mins > 0 {
		return mins
	}
	if val := r.settings.GetResolvedValue("INACTIVITY_TIMEOUT_MINUTES"); val != "" {
